...
TEST(DateProblem, TestaLimitesAnoERRO)
{
  ...
  for (int i = 0; i < (sizeof(vector)/sizeof(vector[0])); i++){
    ...
    TEST_ASSERT_EQUAL_STRING(esperado, NextDate(25, 12, vector[i]));
  }

  TEST_ASSERT_EQUAL_STRING("01/01/1900", NextDate(31, 12, 1899));
  TEST_ASSERT_EQUAL_STRING("01/02/2100", NextDate(31, 1, 2100));
...
TEST(DateProblem, TestaLimitesMesERRO)
{
  ...
  for (int i = 0; i < (sizeof(vector)/sizeof(vector[0])); i++){
    ...
    TEST_ASSERT_EQUAL_STRING(esperado, NextDate(25, vector[i], 2000));
  }
}
...
TEST(DateProblem, TestaLimitesDiaERRO)
{
  ...
  for (int i = 0; i < (sizeof(vector)/sizeof(vector[0])); i++){
    ...
    TEST_ASSERT_EQUAL_STRING(esperado, NextDate(vector[i], 12, 2000));
  }
}
...
{
  TEST_ASSERT_EQUAL_STRING("01/03/2021", NextDate(29, 2, 2021));
  TEST_ASSERT_EQUAL_STRING("01/03/2022", NextDate(29, 2, 2022));
  TEST_ASSERT_EQUAL_STRING("01/03/2023", NextDate(29, 2, 2023));
  TEST_ASSERT_EQUAL_STRING("01/03/2025", NextDate(29, 2, 2025));
